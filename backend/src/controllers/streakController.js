  ...
  try {
    ...
    if (!lastDate) {
      ...
      if (diffDays === 0) {
        ...
        newStreak += 1;
        if (newStreak % 7 === 0) {
          await db.from("users").update({ streak_shield_active: true }).eq("id", req.user.id);
        }
      } else if (diffDays === 2 && (user.streak_shield_active || user.streak_freeze_count > 0)) {
        if (user.streak_shield_active) {
          await db.from("users").update({ streak_shield_active: false }).eq("id", req.user.id);
        } else {
          await db.from("users").update({ streak_freeze_count: Math.max(0, user.streak_freeze_count - 1) }).eq("id", req.user.id);
        }
        newStreak += 1;
      } else {
        newStreak    = 1;
...
export const buyStreakFreeze = async (req, res) => {
  try {
    ...
    const { data: user, error } = await db
      .from("users").select("xp, streak_freeze_count").eq("id", req.user.id).single();
    ...
    await supabase.from("activity_logs").insert({
      user_id: req.user.id, type: "streak_freeze_bought",
...
export const getLeaderboard = async (req, res) => {
  try {
    ...
    const ranked = rows.map((u, i) => ({
      ...
      displayXP:  period === "weekly" ? (u.weekly_xp || 0) : (u.xp || 0),
      level:      u.level || 1,
    ...
    return res.status(200).json({ success: true, data: { board: ranked, myRank: myEntry?.rank || null, myEntry } });
  } catch (error) {
    return res.status(500).json({ success: false, message: error.message });
  }
};

// ─── GET /api/streaks/invite-code ─────────────────────────────────────────────
export const getInviteCode = async (req, res) => {
  try {
    const db = getAuthedSupabaseClient(req.accessToken);

    const { data: user } = await db
      .from("users").select("invite_code").eq("id", req.user.id).single();

    // Generate one if missing
    if (!user?.invite_code) {
      const code = crypto.randomBytes(3).toString("hex").toUpperCase(); // 6-char
      await db.from("users").update({ invite_code: code }).eq("id", req.user.id);
      return res.status(200).json({ success: true, data: { code } });
    }

    return res.status(200).json({ success: true, data: { code: user.invite_code } });
  } catch (error) {
    return res.status(500).json({ success: false, message: error.message });
  }
};
  ...
  try {
    ...
    const { data, error } = await db
      ...
      .select(`
        ...
        friend:friend_id ( id, full_name, username, avatar_url, level, weekly_xp, streak_days, league_tier ),
        requester:user_id ( id, full_name, username, avatar_url )
      `)
      .or(`user_id.eq.${req.user.id},friend_id.eq.${req.user.id}`);
    ...
    const normalised = (data || []).map((f) => {
      const isRequester = f.requester?.id === req.user.id;
      return {
        id:        f.id,
        status:    f.status,
        direction: isRequester ? "sent" : "received",
        friend:    isRequester ? f.friend : f.requester,
      };
    });
...
// ─── GET /api/streaks/friends/search ─────────────────────────────────────────
  ...
  try {
    ...
    if (!targetId && inviteCode) {
      const { data: found } = await supabase
        .from("users").select("id").eq("invite_code", inviteCode.toUpperCase()).single();
      if (!found) return res.status(404).json({ success: false, message: "Invite code not found." });
      targetId = found.id;
    }

    if (!targetId) return res.status(400).json({ success: false, message: "friendId or inviteCode required." });
    if (targetId === req.user.id) return res.status(400).json({ success: false, message: "You can't add yourself." });

    // Check not already friends/pending
    const { data: existing } = await db
      .from("friends")
      .select("id, status")
      .or(`and(user_id.eq.${req.user.id},friend_id.eq.${targetId}),and(user_id.eq.${targetId},friend_id.eq.${req.user.id})`)
      .single();
...
export const respondToFriend = async (req, res) => {
  try {
    ...
    const { error } = await db
      .from("friends").update({ status: "accepted" }).eq("id", id).eq("friend_id", req.user.id);
    if (error) throw error;

    return res.status(200).json({ success: true, message: "Friend accepted! 🎉" });
  } catch (error) {
    return res.status(500).json({ success: false, message: error.message });
  }
};
...
export const weeklyReset = async (req, res) => {
  ...
  try {
    ...
    const { data: allUsers } = await supabase
      .from("users").select("id, weekly_xp, league_tier").order("weekly_xp", { ascending: false });
    ...
    const historyRows = allUsers.map((u, i) => {
      ...
      return { user_id: u.id, week_start: weekStartStr, xp_earned: u.weekly_xp || 0, league_tier: u.league_tier, rank_position: rank, promoted, demoted, _newTier: newTier };
    });
    ...
    for (const row of historyRows) {
      await supabase.from("users").update({ weekly_xp: 0, league_tier: row._newTier }).eq("id", row.user_id);
    }
